...
# Read train and test
train_raw <- fread("~/GitHub/cardif/train.csv", stringsAsFactors=TRUE) 
print(dim(train_raw))
...
test_raw <- fread("~/GitHub/cardif/test.csv", stringsAsFactors=TRUE) 
test_id <- test_raw$ID
test_raw$ID <- NULL
print(dim(test_raw))
...
submission <- read.csv("~/GitHub/cardif/sample_submission.csv")
